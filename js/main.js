...
(function (html) {
  ...
  const ssProjectModals = function () {
    ...
    const restoreContent = function (content) {
      const entry = contentMap.get(content);
      if (!entry) return;
      const { parent, nextSibling, display } = entry;
    ...
    const openModal = function (targetSelector) {
      ...
      const title = content.querySelector(".case-modal__title");
      if (title) {
        if (!title.id) title.id = "project-modal-title";
    ...
    const closeModal = function () {
      ...
      const content = modalContent.firstElementChild;
      if (content) {
